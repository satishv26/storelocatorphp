...
$headerValue = "68e9f227efcc4ead96154d49a154fbea";
    ...
    $range = isset($_GET["range"]) ? mysqli_real_escape_string($conn, $_GET["range"]) : "50";
...
try {
    ...
    if (is_numeric($isVisible) && is_numeric($isActive) && is_numeric($no_of_records_per_page) && is_numeric($pageno) && isset($_GET["sku"]) && !empty($_GET['sku']) && $sku != 'all') {
        if (isset($_GET["range"]) && is_numeric($_GET["range"])) {
            $sql =
                "SELECT * , ($unitMultiplier * 2 * ASIN(SQRT( POWER(SIN(( $latitude1 - latitude) *  pi()/180 / 2), 2) +COS( $latitude1 * pi()/180) * COS(latitude * pi()/180) * POWER(SIN(( $longitude1 - longitude) * pi()/180 / 2), 2) ))) as distance from stores having  distance <= ".$range." AND is_active = ".$isActive." AND is_visible = ".$isVisible." AND store_id IN (SELECT location_id from items WHERE sku = ".$sku." AND quantity > 0) order by distance";
        } else {
            $sql =
                "SELECT * FROM  stores WHERE is_active = ".$isActive." AND is_visible = ".$isVisible." AND store_id IN (SELECT location_id from items WHERE sku = ".$sku." AND quantity > 0)";
        }
    }else{
    ...
    $sql1 =
        "SELECT * FROM  stores WHERE store_id IN ($ids) AND is_active = ".$isActive." AND is_visible = ".$isVisible." AND store_id IN (SELECT location_id from items WHERE quantity > 0 $sku) LIMIT $offset, $no_of_records_per_page";
        $conn->set_charset("utf8");
